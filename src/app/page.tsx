...
// this is a server component now, so no client stuff can happen in here, change it perhaps?
export default async function Home() {
  // if session is null then user is logged out otherwise logged in
  ...
  return (
    ...
    <div>
      <div style={{height: "100%", display: "flex", justifyContent: "center", flexDirection: "column", alignItems: "center"}}>
        ...
        <div style={{overflow: "hidden", background: "linear-gradient(#008476, #d9d9d9)", opacity: 0.1, width: 200, height: "calc(100%)", position: "absolute", transform: "rotate(20deg)", zIndex: 1}}></div>
          ...
          <div>
            <AppBar position="absolute" sx={{ backgroundColor: 'transparent', boxShadow: 'none' }}>
              <Toolbar>
                ...
                {!session
                  ? <div className={styles.loginButton}><a href="/auth/login">Login</a></div>
                  : <><h1>Welcome, {session.user.name}!</h1><a href="/auth/logout">Log out</a></>
            ...
            <div className={styles.topHalfContentsContainer2}>
              <div className={styles.leftSection}>
                <div className={styles.leftSectionTitle}>
                  <span style={{color:'#303030', fontWeight: 600, opacity: 0.8}}>LENORAH</span> <span style={{color:'white', opacity: 0.8}}>Learning</span>
                </div>
                <div className={styles.leftSectionDescription}>
                  Igniting the light within every mind. Helping every student shine, one session at a time—offering online and in-person tutoring in HSC Maths, Economics, Biology, and more.
                </div>
